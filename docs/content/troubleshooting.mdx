...
- Review the log output for the exact query that failed
...
1. **Check sync interval**
   - Default is 6 hours (`GitSync.IntervalSeconds`)
...
1. **Reduce sync frequency**
   - Increase `GitSync.IntervalSeconds` for less frequent repository checks
...
All MCP server and Worker logs go to stderr using Microsoft.Extensions.Logging (Console provider).
