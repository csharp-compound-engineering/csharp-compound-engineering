...
The sync interval is configured globally via `GitSync.IntervalSeconds` (default: 21600 = 6 hours).

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `GitSync.CloneBaseDirectory` | string | `/tmp/compound-docs-repos` | Directory for cloned repositories |
| `GitSync.IntervalSeconds` | integer | `21600` | How often to sync repositories (seconds) |
...
The MCP server uses HTTP transport on port 8080. Add it to your Claude Code configuration:
...
3. Check stderr output for configuration errors at startup
